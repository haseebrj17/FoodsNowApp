...
const Particules = ({ transition, activeTransition }) => {
    ...
    const x = Animated.add(Animated.multiply(transition, SEGMENT), SEGMENT / 2 - size / 2);
    const top = Animated.interpolate(activeTransition, {
        inputRange: [0, 0.5, 1],
        outputRange: [middle, PADDING / 2, middle],
    });
    const bottom = Animated.interpolate(activeTransition, {
        inputRange: [0, 0.5, 1],
        outputRange: [middle, HEIGHT - PADDING / 2, middle],
    });
    const s = Animated.interpolate(activeTransition, {
        inputRange: [0, 0.5, 1],
        outputRange: [0.75, 1, 0.75],
    });
    const opacity = Animated.and(Animated.neq(activeTransition, 0), Animated.neq(activeTransition, 1));
        ...
        <View style={styles.container} pointerEvents="none">
            <View style={styles.particules}>
                ...
                {bottomParticules.map((particule) => {
                    ...
                    const translateX = subParticules.reduce(
                        (acc) => withSpringTransition(acc),
                        Animated.divide(Animated.add(x, withSpringTransition(x)), 2)
                    );
                    const translateY = subParticules.reduce(
                        (acc) => withSpringTransition(acc),
                        Animated.divide(Animated.add(bottom, withSpringTransition(bottom)), 2)
                    );
                    const scale = subParticules.reduce(
                        (acc) => withSpringTransition(acc),
                        Animated.divide(Animated.add(s, withSpringTransition(s)), 2)
                    );
                    return (
                        <Animated.View
                            key={particule}
